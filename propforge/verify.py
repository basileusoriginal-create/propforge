...
def verify_drawable(spec: PropSpec, info: DrawableInfo) -> list[Finding]:
    ...
    for suffix, sampler in ROLE_SAMPLERS.items():
        ...
        if not has_source:
            continue
        # Native parameter hashes are case-insensitive; szio resolves their
        # names in lowercase when a binary is read back as diagnostic XML.
        if sampler.lower() not in {name.lower() for name in info.samplers}:
            add(
                Level.ERROR,
                "sampler_not_bound",
