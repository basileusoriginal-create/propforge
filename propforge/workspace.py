...
class Workspace:
    ...
    def archive(self, job: Job) -> Path:
        ...
        target = _free_name(self.done / job.mesh.name, with_sidecar=True)
...
def _free_name(target: Path, with_sidecar: bool = False) -> Path:
    """Verhindert, dass ein zweiter Durchlauf eine aeltere Fassung ueberschreibt."""
    def available(path: Path) -> bool:
        return not path.exists() and not (with_sidecar and sidecar_for(path).exists())
