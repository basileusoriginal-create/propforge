...
def normalize_lod_normals(mesh: bpy.types.Mesh, label: str) -> None:
    """Preserve directions while repairing non-unit normals after Decimate."""
    normals = [n.vector.copy() for n in mesh.corner_normals]
    bad = [n for n in normals if abs(n.length - 1.0) > 0.0001]
    if not bad:
        return
    if any(n.length < 1e-8 for n in normals):
        raise RuntimeError(f"LOD {label}: Null-Normale nach der Reduktion.")
    mesh.normals_split_custom_set([n.normalized() for n in normals])
    mesh.update()
    log(f"  LOD {label}: {len(bad)} verkuerzte Ecknormalen normalisiert.")
...
def build(job: dict, fmt: str, version: str, render_dir: str | None = None) -> dict:
    ...
    # weit genug weggeht.
    for lod_key, mesh in lod_meshes.items():
        normalize_lod_normals(mesh, lod_key)
