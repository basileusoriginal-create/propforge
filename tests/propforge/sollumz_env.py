...
def module_candidates() -> tuple[str, ...]:
    """Use the enabled add-on, never an inactive copy's Python API."""
    try:
        import bpy
        enabled = tuple(
            name for name in bpy.context.preferences.addons.keys()
            if name in MODULE_CANDIDATES
            or name.rsplit(".", 1)[-1].lower() in ("sollumz", "sollumz_dev")
        )
    except ImportError:
        return MODULE_CANDIDATES
    if not enabled:
        raise ImportError("Kein Sollumz-Add-on ist im Blender-Startkontext aktiviert.")
    if len(enabled) != 1:
        raise ImportError("Mehrere Sollumz-Add-ons aktiviert: " + ", ".join(enabled))
    return enabled
...
try:
    ...
    if "target_formats" not in properties or "target_versions" not in properties:
        raise ImportError("Sollumz-Export-API benoetigt target_formats und target_versions (2.9-dev).")
    found = api.module
except Exception as exc:
    print("SOLLUMZ_ERROR=" + str(exc).replace("\\n", " | "))
print("SOLLUMZ=" + found)
