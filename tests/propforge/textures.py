...
def to_power_of_two(img: Image.Image, maximum: int = 2048) -> Image.Image:
    ...
    """
    if min(img.size) < VALID_TEXTURE_SIZES[0]:
        raise TextureError("Texturkanten unter 16 px werden nicht automatisch hochskaliert.")
    target_w = nearest_power_of_two(img.width, min(maximum, img.width))
    target_h = nearest_power_of_two(img.height, min(maximum, img.height))
