...
def inspect(path: str | Path, name: str) -> tuple[AssetInfo, np.ndarray, np.ndarray]:
    ...
    if not info.is_centered:
        info.notes.append(
            "Die Bounding-Box-Mitte liegt nicht bei Null. Ein Standflaechen- "
            "oder Anschlussursprung kann trotzdem korrekt sein; center bewusst waehlen."
        )
...
def config_snippet(info: AssetInfo, mesh_path: Path, texture_dir: Path,
    ...
    if not info.is_centered:
        lines.append('# Vorschlag fuer Bodenobjekte; gewollten Anschlussursprung separat pruefen.')
