...
class TestArchive:
    def test_repeated_archive_preserves_mesh_sidecar_pair(self, tmp_path):
        w = ws.Workspace.load(tmp_path)
        w.ensure()
        for profile in ("standard", "detailed"):
            mesh = glb(w.inbox)
            job = ws.Job(name=mesh.stem, mesh=mesh, profile=profile)
            job.write()
            target = w.archive(job)
            assert ws.read_job(target).profile == profile
        assert (w.done / "pf_flasche_2.job.json").is_file()

    def test_orphan_sidecar_is_not_overwritten(self, tmp_path):
        w = ws.Workspace.load(tmp_path)
        w.ensure()
        old = w.done / "pf_flasche.job.json"
        old.write_text('{"profile":"hero"}')
        mesh = glb(w.inbox)
        job = ws.Job(name=mesh.stem, mesh=mesh)
        job.write()
        target = w.archive(job)
        assert target.name == "pf_flasche_2.glb"
        assert json.loads(old.read_text())["profile"] == "hero"
