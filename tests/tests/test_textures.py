...
class TestPowerOfTwo:
    ...
    def test_does_not_enlarge_small_source_to_profile_limit(self):
        img = Image.new("RGB", (256, 128))
        assert textures.to_power_of_two(img, maximum=1024) is img

    def test_rejects_undersized_edge_instead_of_upscaling(self):
        with pytest.raises(textures.TextureError, match="hochskaliert"):
            textures.to_power_of_two(Image.new("RGB", (8, 32)))
