...
def test_convert_preserves_origin_and_archives_only_after_success(
    ...
    # Der Fehler, der diesen Fall gebaut hat, zeigte sich nur mit einem
    # Terminal an stdin. Also wird eines vorgetaeuscht, statt sich darauf zu
    # verlassen, wie die Testumgebung gerade gestartet wurde. Fragt die
    # Umwandlung trotz vorhandener Begleitdatei nach, faellt das jetzt auf.
    monkeypatch.setattr(cli.sys, "stdin", SimpleNamespace(isatty=lambda: True))
    def never(*a, **k):
        raise AssertionError("Nachgefragt, obwohl eine Begleitdatei vorliegt.")
    monkeypatch.setattr(cli, "_ask_profile", never)
    monkeypatch.setattr(cli, "_ask_material", never)
    stages = []
    def stage(name):
        def run(args):
    ...
    # no_ask=False ist der Normalfall der Kommandozeile. Weil jeder Auftrag hier
    # eine Begleitdatei hat, darf trotzdem nichts gefragt werden - genau das
    # prueft der Fall mit, wenn stdin ein Terminal ist.
    code = cli.cmd_convert(argparse.Namespace(
        root=tmp_path, blender="blender", texconv=None, format="NATIVE", no_ask=False))
