...
tmp_path, capsys und monkeypatch), parametrize (auch gestapelt), raises,
approx. Auf einer normalen Entwicklungsmaschine
...
class _MonkeyPatch:
    """Ersatz fuer pytests monkeypatch: setattr/setitem/delattr mit Ruecknahme.

    Die Ruecknahme ist der eigentliche Zweck. Ohne sie sickert ein
    ausgetauschtes Modul (etwa ein vorgetaeuschtes bpy in sys.modules) in
    alle folgenden Testfaelle und macht die Ergebnisse wertlos.
    """
    ...
    def setitem(self, mapping, key, value):
        old = mapping[key] if key in mapping else self._MISSING
...
class _Mark:
    ...
    def parametrize(argnames, argvalues):
        ...
        def decorator(func):
            ...
            existing = getattr(func, "__parametrize__", None)
            if existing is None:
                func.__parametrize__ = (names, values)
                return func
            # Gestapelte Dekoratoren: pytest bildet das kartesische Produkt.
            # Ohne das wuerde der aeussere den inneren ueberschreiben, und
            # der Testfall bekaeme fehlende Argumente.
            outer_names, outer_values = existing
            func.__parametrize__ = (
                outer_names + names,
                [o + n for n in values for o in outer_values],
            )
            return func
        return decorator
...
def _resolve_args(func, fixtures: dict, tmp_root: Path, cache: dict):
    ...
    for param in inspect.signature(func).parameters:
        ...
        if param == "monkeypatch":
            ...
            kwargs["monkeypatch"] = cache["monkeypatch"]
        elif param == "capsys":
